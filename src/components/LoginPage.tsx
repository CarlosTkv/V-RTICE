...
export const LoginPage: React.FC<LoginPageProps> = ({ onLogin, onBackToLanding }) => {
  ...
  useEffect(() => {
    const parseHash = () => {
      ...
      } else if (action === 'mtls-login' && token) {
        handleMtlsLogin(token);
      }
    };
  ...
  // Handle retorno do túnel mTLS da Oracle Cloud VPS
  const handleMtlsLogin = (token: string) => {
    setIsLoading(true);
    setErrorMessage('');
    
    setTimeout(() => {
      setIsLoading(false);
      // Aqui, o token poderia ser um JWT seguro assinado pela VPS.
      // Para demonstração imediata/mock, tentamos autenticar assumindo que o token
      // contém o serialNumber ou ID do certificado autorizado retornado pelo túnel.
      
      try {
        // Tentativa de parse se o token for um base64 JSON com dados do certificado
        const decodedString = atob(token);
        const certData = JSON.parse(decodedString);
        
        const res = AuthService.loginWithCertificate(certData as DigitalCertificateInfo);
        if (res.success && res.user) {
          setIsLoggingInSuccess(true);
          setSuccessMessage(`Certificado Digital autenticado via mTLS (Oracle Cloud) com sucesso! Bem-vindo, ${res.user.name}`);
          setPendingAuthUser(res.user);
          setIsLoggingInSplash(true);
        } else {
          setErrorMessage(res.error || 'Acesso Negado: O certificado mTLS interceptado não possui vínculo com uma conta ativa.');
        }
      } catch (e) {
        // Fallback: tratar o token como um Serial Number ou ID direto (ex: para testes locais ou dev)
        const res = AuthService.loginWithCertificate(token);
        if (res.success && res.user) {
          setIsLoggingInSuccess(true);
          setSuccessMessage(`Certificado Digital autenticado via mTLS (Oracle Cloud) com sucesso! Bem-vindo, ${res.user.name}`);
          setPendingAuthUser(res.user);
          ...
          setErrorMessage(res.error || 'Falha ao processar o ticket de autenticação mTLS recebido da Oracle Cloud.');
        }
      }
      
      // Limpar a URL para não manter o token exposto
      ...
      <div className={`w-full ${activeTab === 'plans' || activeTab === 'complete_admin_registration' ? 'max-w-5xl' : 'max-w-md sm:max-w-xl'} bg-[#0F172A]/90 backdrop-blur-2xl border border-slate-700/80 rounded-3xl shadow-2xl overflow-hidden relative z-10 transition-all duration-300`}>
        ...
        <div className="p-6 sm:p-7 border-b border-slate-700/80 bg-gradient-to-b from-[#0B0F19]/90 to-[#0F172A]/90 flex flex-col items-center text-center">
          ...
          <p className="text-xs text-slate-300 mt-2 max-w-sm leading-relaxed font-medium">
            Ecossistema de Regimes Tributários, Reforma 2026-2033, Pareceres Oficiais e Gestão Fiscal
          </p>
        </div>
            ...
            <div className="space-y-4">
              ...
              {loginMethod === 'certificate' && (
                <div className="space-y-5 animate-in fade-in py-2">
                  ...
                  {/* Se já foi carregado ou selecionado um certificado do computador local (fallback) */}
                  {customCertLoaded ? (
                    ...
                    <div className="space-y-3 pt-1">
                      {/* Opção Principal: Túnel mTLS Oracle Cloud */}
                      <button
                        type="button"
                        onClick={() => {
                          const returnUrl = encodeURIComponent(window.location.origin + '/#action=mtls-login');
                          window.location.href = `https://certificado.verticeanalises.com.br/login?redirect=${returnUrl}`;
                        }}
                        className="w-full py-4 px-4 bg-gradient-to-r from-emerald-600 via-teal-600 to-blue-600 hover:opacity-95 text-white font-extrabold text-xs sm:text-sm rounded-xl flex items-center justify-center gap-2.5 transition shadow-lg shadow-emerald-900/40 cursor-pointer transform hover:scale-101"
                      >
                        <HardDrive className="w-5 h-5 text-emerald-200 shrink-0" />
                        <span>Usar Token A3 / Gerenciador do Computador</span>
                      </button>

                      <div className="pt-2 pb-1 text-center flex items-center gap-3">
                        <div className="flex-1 border-t border-slate-800"></div>
                        <span className="text-[9px] text-slate-500 uppercase tracking-wider font-bold">Upload Manual (P12/PFX)</span>
                        <div className="flex-1 border-t border-slate-800"></div>
                      </div>

                      {/* Opção 2: Seleção Direta de Arquivo .PFX/.P12 */}
                      <button
                        type="button"
                        onClick={() => certFileInputRef.current?.click()}
                        className="w-full py-3 px-4 bg-slate-900 hover:bg-slate-800 border border-slate-700 hover:border-emerald-500/50 text-slate-300 font-bold text-xs rounded-xl flex items-center justify-center gap-2 transition cursor-pointer"
                      >
                        <FolderOpen className="w-4 h-4 text-emerald-400 shrink-0" />
                        <span>Enviar arquivo local (.pfx / .p12 / A1)</span>
                      </button>
                    </div>
                  )}
                </div>
      ...
      {isCertModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-[#111827] border border-slate-700/80 rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden text-slate-100 flex flex-col">
            ...
            {/* Searching/Loading Animation Overlay */}
            <div className="bg-emerald-950/20 border-b border-emerald-500/20 px-6 py-2 flex items-center gap-2">
              <RefreshCw className="w-3.5 h-3.5 text-emerald-400 animate-spin" />
              <span className="text-[10px] font-bold text-emerald-300 uppercase tracking-widest">Buscando certificados instalados no dispositivo...</span>
            </div>
